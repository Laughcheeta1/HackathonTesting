...
function jsonParams() {
    return {
        ...
        tags: { project: "German", action: "bootstrap" },
    };
}

function multipartParams(extraHeaders = {}) {
    return {
        headers: {
            Accept: "application/json",
            ...extraHeaders,
...
function getAuthToken(userId) {
    const response = http.post(url("/auth/token"), JSON.stringify({ user_id: userId }), jsonParams());
    check(response, {
        "bootstrap auth token created": (r) => r.status === 200,
    });
...
function uploadVideo(index, userId) {
    ...
    const response = http.post(
        ...
        multipartParams({ Authorization: `Bearer ${token}` }),
    );
...
function addComment(index, userId, videoId) {
    const token = getAuthToken(userId);
    if (!token) return;

    const response = http.post(
        url(`/videos/${videoId}/comments`),
        JSON.stringify({ content: `comment-${index}` }),
        {
            headers: {
                Accept: "application/json",
                "Content-Type": "application/json",
...
export default function bootstrap() {
    ...
    for (let index = 1; index <= COMMENT_COUNT; index += 1) {
        ...
        addComment(index, userId, videoId);
