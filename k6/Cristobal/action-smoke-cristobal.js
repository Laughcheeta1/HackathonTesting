...
function firstExistingUserId() {
    const response = http.get("http://localhost:8000/users?limit=500&offset=0", {
        tags: { project: "Cristobal", action: "smokeSetup", endpoint: "listUsers" },
    });
...
function hydrateRepositoryFromExistingVideos() {
    ...
    const response = http.get("http://localhost:8000/videos?limit=100&offset=0", {
        tags: { project: "Cristobal", action: "smokeSetup", endpoint: "listVideos" },
    });
...
export default function runSelectedAction() {
    ...
    throw new Error(`Unknown Cristobal smoke action: ${action}`);
}
