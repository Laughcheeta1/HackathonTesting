...
function loadSeedManifest(path = "./seed-manifest-cristobal.json") {
    let manifest;
    try {
        manifest = JSON.parse(open(path));
    } catch (error) {
        throw new Error(`Could not load Cristobal seed manifest at ${path}. Run bootstrap first from the repo root so it writes k6/Cristobal/seed-manifest-cristobal.json, or pass SEED_MANIFEST=<path>.`);
    }
...
function pickRandomUploadedVideoSelection() {
    for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        const response = http.get(
            url(`/videos?limit=100&offset=${offset}`),
